...
export default function ExpenseTracking() {
  ...
  const [form, setForm] = useState({ category: 'utilities', amount: 0, date: new Date().toISOString().slice(0, 10), vendor: '', description: '', recurring: false, recurring_period: 'none', payment_method: 'bank_transfer' });
  ...
  const save = async () => {
    if (form.amount <= 0 || !form.date) { toast({ title: 'Amount and date required', variant: 'destructive' }); return; }
    try {
      await db.entities.Expense.create({ ...form, store_id: currentStore.id, approved_by: user?.full_name || 'Manager' });
      toast({ title: 'Expense logged' });
      setDialogOpen(false);
      setForm({ category: 'utilities', amount: 0, date: new Date().toISOString().slice(0, 10), vendor: '', description: '', recurring: false, recurring_period: 'none', payment_method: 'bank_transfer' });
      load();
    ...
    <div className="p-6 space-y-4 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Log daily overheads, utilities, and maintenance to calculate net profitability</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card><CardContent className="p-4">
          <div className="flex items-center gap-2 text-slate-500 text-sm"><TrendingUp className="w-4 h-4" /> Revenue</div>
        ...
        <Card><CardContent className="p-4">
          <div className="flex items-center gap-2 text-slate-500 text-sm"><TrendingDown className="w-4 h-4" /> Net Profit</div>
        ...
        <Card><CardContent className="p-4">
          ...
          <div className={`text-2xl font-bold ${margin >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{margin.toFixed(1)}%</div>
        </CardContent></Card>
      </div>
              ...
              {form.recurring && (
                <div>
                  ...
                  <Select value={form.recurring_period} onValueChange={(v) => setForm({ ...form, recurring_period: v })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{['weekly', 'monthly', 'quarterly', 'yearly'].map(p => <SelectItem key={p} value={p} className="capitalize">{p}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
              )}
            </div>
